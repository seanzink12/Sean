...
description: Complete Apple.com design system reference - colors, typography (SF Pro Display/Text), spacing, component specs, elevation rules, border radii, and responsive breakpoints for a photography-first interface with a single Action Blue (#0066cc) accent. Use this skill whenever building or styling UI in this project that should match Apple's design language - marketing pages, product tiles, buttons, nav bars, cards, or any web/frontend work where an "Apple-style" or minimal, photography-first aesthetic is wanted. Consult before inventing colors, fonts, spacing, or component styles from scratch.
